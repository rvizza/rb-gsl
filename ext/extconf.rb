...
def gsl_config()
  ...
  IO.popen("#{GSL_CONFIG} --cflags") do |f|
    ...
    puts(cflags)
    $CFLAGS += " " + cflags
    ($CFLAGS += " " + ENV["CPPFLAGS"]) if ENV.key?( "CPPFLAGS" )
  ...
  IO.popen("#{GSL_CONFIG} --libs") do |f|
    ...
    ($LOCAL_LIBS += " " + ENV["LD_LIBRARY_PATH"]) if ENV.key?( "LD_LIBRARY_PATH" )
...
begin
  require 'rubygems'
  na_gemspec = if Gem::Specification.respond_to?(:find_by_path)
    Gem::Specification.find_by_path('narray.h')
  else
    Gem.searcher.find('narray.h')
  end
  if na_gemspec
